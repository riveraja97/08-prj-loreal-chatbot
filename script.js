  ...
  chatForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (userContext.pastQuestions.length > MAX_HISTORY) {
        userContext.pastQuestions = userContext.pastQuestions.slice(
          -MAX_HISTORY
        );
      }
      ...
      if (maybeName && (!userContext.name || userContext.name !== maybeName)) {
        ...
        appendMessageToDOM("assistant", ack, false, ackTs).classList.add(
          "meta"
        );
        conversation.push({
          role: "assistant",
        ...
        if (parsed && Array.isArray(parsed.recommendations)) {
          ...
          if (loadingEl) {
            ...
            if (ts) {
              ...
              ts.textContent = `${String(d.getHours()).padStart(
                2,
                "0"
